...
document.addEventListener("DOMContentLoaded", () => {
  // Se espera que el header se cargue dinámicamente y tenga la clase "header-section"
  ...
  window.addEventListener("scroll", () => {
    ...
    // Si se supera el umbral, añade la clase sticky (cambia la posición a fixed)
    ...
    // Ocultar el header al hacer scroll hacia abajo y mostrarlo al subir (solo si se supera el umbral)
    if (currentScroll > lastScroll && currentScroll > threshold) {
      header.style.top = "-60px"; // Oculta el header; ajusta este valor según la altura real
    } else {
      header.style.top = "0";
    }
...
window.addEventListener("scroll", () => {
  ...
  if (currentScroll === 0) {
    // Cuando estamos en la parte superior, aseguramos que el header se muestre
    header.classList.remove("hide");
  } else if (currentScroll > lastScroll) {
    // Si se desplaza hacia abajo, añadimos la clase 'hide'
    header.classList.add("hide");
  } else if (currentScroll < lastScroll) {
    // Si se desplaza hacia arriba, removemos la clase 'hide'
    header.classList.remove("hide");
  }
