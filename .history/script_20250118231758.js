...
// Inicializa Swiper con configuración de parallax
    ...
    on: {
        slideChangeTransitionStart: function () {
            ...
            slides.forEach((slide, index) => {
                const progress = slide.swiperSlideOffset || 0; // Calcula el desplazamiento
                const translateX = progress * 0.3; // Ajusta el valor del parallax
                const translateY = progress * 0.1; // Ajusta el desplazamiento vertical
                const scale = 1 + Math.abs(progress * 0.0005); // Escala sutil basada en la posición

                // Aplica transformaciones a cada imagen estática y activa
                const staticImg = slide.querySelector(".static-img");
                const activeGif = slide.querySelector(".active-gif");

                if (staticImg) {
                    staticImg.style.transform = `translate(${translateX}px, ${translateY}px) scale(${scale})`;
                }
                if (activeGif) {
                    activeGif.style.transform = `translate(${translateX}px, ${translateY}px) scale(${scale})`;
                }
            });
        },
        slideChangeTransitionEnd: function () {
            // Restablece las transformaciones al final de la transición
            const slides = document.querySelectorAll(".swiper-slide");
            slides.forEach((slide) => {
                ...
                if (staticImg) {
                    staticImg.style.transform = "translate(0, 0) scale(1)";
                }
                if (activeGif) {
                    activeGif.style.transform = "translate(0, 0) scale(1)";
                }
            });
        },
    },
