...
new CursorFx({
    mixBlendMode: true, // Permite mezcla de colores con el fondo
    shape: 'circle', // Usa la forma circular como base
...
document.addEventListener('mouseover', (e) => {
    const isInteractive = getComputedStyle(e.target).cursor === 'pointer';
    if (isInteractive) {
        cursor.classList.add('pointer'); // Añadir efecto de hover
    }
});

document.addEventListener('mouseout', () => {
    cursor.classList.remove('pointer'); // Quitar efecto
});
const buttons = document.querySelectorAll('.btn'); // Selecciona los botones interactivos
...
buttons.forEach((button) => {
    ...
    button.addEventListener('mouseleave', () => {
        cursor.style.width = '40px'; // Tamaño inicial
