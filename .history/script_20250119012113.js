...
// Ensure CursorFx is defined before initializing
if (typeof CursorFx !== 'undefined') {
    // Inicializar Cursor.js con configuración básica
    new CursorFx({
        mixBlendMode: true, // Mezcla de colores con el fondo
        shape: 'circle', // Forma base del cursor
        elementSize: 40, // Tamaño base del cursor
    });
} else {
    console.error('CursorFx is not defined. Please include the CursorFx library.');
}

// Selecciona todos los botones interactivos en el DOM
const buttons = document.querySelectorAll('.btn');

// Detectar interacción con botones y ajustar el cursor
buttons.forEach((button) => {
    button.addEventListener('mouseenter', () => {
        const rect = button.getBoundingClientRect();

        // Ajustar el cursor para que rodee el botón
        ...
        cursor.style.transform = `translate(-50%, -50%)`; // Centrar el cursor
    });

    button.addEventListener('mouseleave', () => {
        const cursor = document.querySelector('.cursor-fx');
        cursor.style.width = '40px'; // Tamaño original
        cursor.style.height = '40px';
        cursor.style.transform = `translate(-50%, -50%) scale(1)`; // Resetear transformación
    });
});

// Reposicionar el cursor en cada movimiento del mouse
...
document.addEventListener('touchend', () => {
    const cursor = document.querySelector('.cursor-fx');
    cursor.style.display = 'none'; // Oculta el cursor al terminar el toque
});
