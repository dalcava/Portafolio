...
document.addEventListener("mousemove", (e) => {
    ...
    // Verifica si el cursor está sobre un botón
    if (target.classList.contains("swiper-button-prev") || 
        target.classList.contains("swiper-button-next") || 
        target.classList.contains("swiper-pause") || 
        target.classList.contains("next-project-btn")) {
        
        const rect = target.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;

        // Ajusta el cursor alrededor del botón
        customCursor.style.transform = `translate(-50%, -50%) scale(1.5)`;
        customCursor.style.left = `${centerX}px`;
        customCursor.style.top = `${centerY}px`;
        customCursor.style.backgroundColor = "rgba(255, 255, 255, 0.8)"; // Cambia el color si es necesario
    } else {
        // Resetea el cursor
...
// Oculta el cursor en touch events
