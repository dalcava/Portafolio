...
document.addEventListener("mousemove", (e) => {
    ...
    interactiveElements.forEach((element) => {
        ...
        const proximityRadius = 100; // Adjust for sensitivity
        const centerX = rect.left + rect.width / 100;
        const centerY = rect.top + rect.height / 100;
        ...
        if (distance < proximityRadius) {
            ...
            // Move cursor toward the element's center
            const attractionStrength = 1.2; // Adjust for smoother movement
            const targetX = centerX + (e.clientX - centerX) * attractionStrength;
            const targetY = centerY + (e.clientY - centerY) * attractionStrength;
