...
window.onload = () => {
    ...
    .fromTo(
        ".icon-button",
        { opacity: 0, y: 0 }, // Start invisible and slightly below
        { opacity: 1, y: 0, duration: 0.25, easeout: "power4.out", stagger: 0 },
        "-=0.5" // Fade and move in
         // Start overlapping with the previous animation
    )
/*     .fromTo(
        ".icon-button",
...
/*    .fromTo(
       ".personal-photo",
       { clipPath: "inset(100% 0 0 0)" },
       { clipPath: "inset(0% 0 0 0)", duration: 1.2, ease: "power4.out" stagger: 0 },
       "-=2.5"
    ) */
    .fromTo(
        ".swiper-slide",
        { opacity: 0, y: 200 }, // Start invisible and slightly below
        { opacity: 1, y: 0, duration: 1, easeout: "power4.out", stagger: 0.5 } // Fade and move in
         // Start overlapping with the previous animation
    )
    .fromTo(
        ".main-title",
