...
// Set up the ScrollTrigger for the swiper container
// Register ScrollTrigger with GSAP
gsap.registerPlugin(ScrollTrigger);

// Initialize Scroll-Linked Slide Navigation
let isAnimating = false; // To prevent rapid navigation on continuous scroll

// Handle scroll events with ScrollTrigger
ScrollTrigger.create({
    trigger: ".boolean-container", // Target the container of the swiper
    start: "top top", // When the container hits the top of the viewport
    end: "bottom bottom", // When the container leaves the viewport
    scrub: false, // No scrub for precise slide switching
    onUpdate: (self) => {
        if (isAnimating) return; // Prevent multiple animations during scroll
        ...
        // Allow animation to finish before allowing the next scroll-triggered slide change
        setTimeout(() => (isAnimating = false), 800); // Adjust delay to match Swiper's speed
    },
});
