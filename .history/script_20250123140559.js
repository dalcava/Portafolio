...
// Ensure Swiper navigation works with scroll
let isAnimating = false; // Prevent rapid-fire slide changes
...
const navigateSwiper = (direction) => {
    ...
    setTimeout(() => (isAnimating = false), 800); // Match Swiper's speed
};

// ScrollTrigger setup for `.boolean-container`
ScrollTrigger.create({
    trigger: ".boolean-container",
    start: "top top", // Start when container hits the top of the viewport
    end: "bottom bottom", // End when container leaves the viewport
    scrub: false, // Discrete steps
    markers: true, // Debug markers for testing
    onUpdate: (self) => {
        navigateSwiper(self.direction); // Trigger Swiper navigation
    },
});

// Optional: Log scroll activity for debugging
window.addEventListener("scroll", () => {
    console.log("Scrolling detected!");
});
