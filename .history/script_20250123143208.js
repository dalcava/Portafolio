...
window.onload = () => {
    // Add shimmer class for an icon (if applicable)
    ...
    timeline
        .fromTo(
            "#canvas1",
            { opacity: 0 },
            { opacity: 1, duration: 1.75, ease: "power4.out" }
        )
        .fromTo(
            ".icon-button",
            { opacity: 0, y: 20 },
            { opacity: 1, y: 0, duration: 0.5, ease: "power4.out", stagger: 0.2 },
            "-=1"
        )
        .fromTo(
            ".main-title",
    ...
    // Function to handle Swiper navigation
    const navigateSwiper = (direction) => {
        ...
        setTimeout(() => (isAnimating = false), 800); // Debounce
    };

    // ScrollTrigger for Swiper navigation
    ScrollTrigger.create({
        trigger: ".boolean-container",
        start: "top top",
        end: "bottom bottom",
        scrub: false,
        onUpdate: (self) => navigateSwiper(self.direction),
    });
