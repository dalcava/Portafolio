...
const scrollRangeStart = 0; // Start of the range
const scrollRangeEnd = 200; // End of the range

// Function to toggle overflow based on the scroll range
const toggleOverflowByRange = () => {
    const currentScroll = window.scrollY || document.documentElement.scrollTop;

    if (currentScroll >= scrollRangeStart && currentScroll <= scrollRangeEnd) {
        // Disable scrolling
...
window.addEventListener("scroll", toggleOverflowByRange);

// Trigger the function once on load to ensure initial state
toggleOverflowByRange(); 