...
const scrollLimit = 200; // Define the scroll range in pixels
let isOverflowEnabled = true; // Track the current overflow state

// Function to toggle overflow
const toggleOverflow = (enable) => {
    if (enable !== isOverflowEnabled) {
        document.body.style.overflow = enable ? "auto" : "hidden";
        document.documentElement.style.overflow = enable ? "auto" : "hidden";
        isOverflowEnabled = enable;
    }
};
...
window.addEventListener("scroll", () => {
    ...
    if (scrollY <= scrollLimit) {
        toggleOverflow(false); // Disable scrolling within range 0 to 200
    } else {
        toggleOverflow(true); // Enable scrolling beyond 200
    }
}); 

console.log(`ScrollY: ${scrollY}, Overflow: ${isOverflowEnabled ? "auto" : "hidden"}`);
