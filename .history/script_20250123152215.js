...
const scrollLimit = 200; // Define the range (0 to 200)
...
const handleWheelScroll = (e) => {
    ...
    scrollValue = Math.max(0, Math.min(scrollValue, scrollLimit)); // Clamp to 0–200

    // Toggle overflow based on scrollValue
    if (scrollValue < scrollLimit || window.scrollY === 0) {
        toggleOverflow(false); // Lock scrolling
    } else {
        toggleOverflow(true); // Unlock scrolling
    }

    // Debug: Log the current scrollValue and scrollY
    console.log("Scroll Value:", scrollValue, "ScrollY:", window.scrollY);
};

// Add wheel event listener
window.addEventListener("wheel", handleWheelScroll, { passive: false });

// Ensure toggleOverflow is set when returning to top
window.addEventListener("scroll", () => {
    if (window.scrollY === 0) {
        toggleOverflow(false); // Lock scrolling at the top
        scrollValue = 0; // Reset scrollValue to initial state
    }
});
