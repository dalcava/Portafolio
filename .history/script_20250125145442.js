...
swiper.on('slideChangeTransitionStart', () => {
    ...
    bullets.forEach((bullet, index) => {
        if (index === swiper.activeIndex) {
            bullet.style.animation = `bullet-timer ${swiper.params.autoplay.delay / 1000}s linear`;
        } else {
...
// Reset animation on autoplay restart
swiper.on('autoplay', () => {
    const bullets = document.querySelectorAll('.swiper-pagination-bullet');
    bullets.forEach((bullet, index) => {
        if (index === swiper.activeIndex) {
...
/*             { opacity: 0, x: -300 }, // Start invisible and slightly below
        ...
        .fromTo(
            ...
            { opacity: 1, y: -35, x:0, duration: 0.85, ease: "power4.out", stagger: 0.25 },
            "-=2" // Fade and move in
        )
        .fromTo(
            ".about-me-compliment",
