...
function init() {
    ...
    const size = Math.min(canvas.width, canvas.height) * 0.002;

    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            let x = col * gridWidth + gridWidth / 2;
            let y = row * gridHeight + gridHeight / 2;
            particlesArray.push(new Particle(x, y, 0, 0, size, '#DEE1E4'));
        }
    }
}
...
window.addEventListener(
    ...
    () => {
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        mouse.radius = (canvas.height / 100) * (canvas.width / 85);
        init();
    },
    { passive: true }
);
