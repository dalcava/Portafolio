...
// Store references to "magnet" (interactive) elements
...
// Magnetic radius (bigger for stronger effect)
const MAGNETIC_RADIUS = 200;

// ==================
// 2) ANIMATION LOOP
// ==================
// Smoothly move the cursor from currentX/Y to targetX/Y
function animateCursor() {
  // Adjust '0.1' or '0.15' to control how fast the cursor catches up
  const ease = 0.15;
...
// Single mousemove listener handles shape changes + sets "target" position
document.addEventListener("mousemove", (e) => {
  const { clientX, clientY, target } = e;

  // Default: follow actual mouse position
  let newX = clientX;
  let newY = clientY;

  // Check proximity to any "magnetic" interactive element
  // (Use a bigger radius for stronger effect)
  ...
  interactiveElements.forEach((element) => {
    ...
    if (distance < MAGNETIC_RADIUS) {
      ...
      // Pull strength: near the center => stronger
      const pullStrength = Math.min(1, 1 - distance / MAGNETIC_RADIUS);
      newX = centerX + (clientX - centerX) * pullStrength;
      newY = centerY + (clientY - centerY) * pullStrength;
    }
  });
  ...
  // ======= Shape & Appearance Logic =======
  // We'll update the cursor's shape/color here, but *not* its position.
  ...
  // 2. Swiper slides (e.g., .active-gif or .imagen-contenida)
  else if (
    target.classList.contains("active-gif") ||
    target.classList.contains("imagen-contenida")
  ) {
    customCursor.style.transform = "translate(-50%, -50%) scale(1.5)";
    customCursor.style.backgroundColor = "rgba(188, 36, 74, 0.15)";
    customCursor.style.borderRadius = "50%";
    customCursor.style.transition = "all 0.15s ease-out";
    customCursor.style.zIndex = "1000";
  }

  // 3. Cursor is "pointer" => enlarge + highlight
  ...
  // 4. Text-like elements => tall, thin cursor
  else if (
...
// Mouseover/mouseout to toggle ".pointer" if desired
