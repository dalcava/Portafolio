...
const interactiveElements = document.querySelectorAll(
  "a, button, .swiper-pagination-bullet, .next-project-btn, .swiper-button-next, .swiper-button-prev, .swiper-pagination-bullet"
);

/**
 * Helper function to apply common style changes to the custom cursor.
 * Accepts a style object with CSS properties to apply.
 */
function applyCursorStyles(styles = {}) {
    Object.entries(styles).forEach(([property, value]) => {
      customCursor.style[property] = value;
    });
  }

/**
 * Reset cursor to its default appearance.
 */
...
/**
 * Track the cursor position and update styles based on target element and proximity.
 */
document.addEventListener("mousemove", (e) => {
  ...
  // (applied only if within a certain proximity)
  interactiveElements.forEach((element) => {
    const rect = element.getBoundingClientRect();
    const proximityRadius = 300;
    const centerX = rect.left + rect.width / 1;  // adjust as needed
    const centerY = rect.top + rect.height / 1; // adjust as needed
    ...
    if (distance < proximityRadius) {
      ...
      const pullStrength = Math.min(50, 1 - distance / proximityRadius);
      const targetX = centerX + (clientX - centerX) * pullStrength;
      const targetY = centerY + (clientY - centerY) * pullStrength;
  ...
  // If not near an interactive element, just follow cursor
  if (!isNearInteractive) {
    applyCursorStyles({
      left: `${clientX}px`,
      top: `${clientY}px`,
      transform: "translate(-50%, -50%) scale(1)",
      backgroundColor: "#BC244A", // Default color
      transition: "transform 0.2s ease-out, background-color 0.3s ease",
    });
  }

  // Now handle specific elements for resizing/recoloring/etc.
  // (The logic below will override any "magnet" positioning if applicable.)

  // Cache the computed cursor style once
  ...
  if (target.classList.contains("swiper-pagination-bullet")) {
    ...
    applyCursorStyles({
      ...
      borderRadius: "24px",
      transition:
        "transform 0.15s ease-out, width 0.1s ease, background-color 0.3s ease, border-radius 0.3s ease",
      zIndex: "2",
    });
  }
  // 2. Swiper slide images (e.g. .active-gif or .imagen-contenida)
  else if (
  ...
  // 3. Pointer elements (cursor: pointer)
  ...
  // 4. Text elements or cursor set to 'text'
  else if (
    computedCursor === "text" ||
...
/**
 * Toggle the "pointer" class on mouseover or mouseout
 * (If you specifically need a .pointer CSS style, you can keep this event.)
 */
