...
class Particle {
    ...
    draw() {
        ctx.beginPath();
        const capsuleWidth = this.size * 3; // Adjust width for the capsule
        const capsuleHeight = this.size * 1.5; // Adjust height for the capsule
        const radius = this.size * 0.75; // Corner radius for smooth edges
    
        ctx.moveTo(this.x - capsuleWidth / 2 + radius, this.y - capsuleHeight / 2);
        ctx.arcTo(
            this.x + capsuleWidth / 2,
            this.y - capsuleHeight / 2,
            this.x + capsuleWidth / 2,
            this.y + capsuleHeight / 2,
            radius
        );
    ...
    update() {
        ...
        if (distance < mouse.radius + this.size) {
            ...
            let repelForce = 1.75; // Reduced repulsion force
...
function init() {
    ...
    let size = Math.min(canvas.width, canvas.height) * 0.0035; // Tamaño proporcional al canvas
...
document.addEventListener("mousemove", (e) => {
    ...
    // Check if the target is a swiper slide
        ...
        // Adjust cursor position and appearance
        customCursor.style.backgroundColor = "rgba(188, 36, 74, 0.250)";
