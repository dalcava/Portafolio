...
class Particle {
    constructor(x, y, directionX, directionY, size, color) {
        ...
        this.size = size;
        this.color = '#F6F6F6'; // Updated color
    }
...
function init() {
    ...
    let size = Math.min(canvas.width, canvas.height) * 0.025; // Tamaño proporcional al canvas
    ...
    for (let row = 0; row < numberOfRows; row++) {
        for (let col = 0; col < numberOfCols; col++) {
            ...
            let directionX = moveHorizontal ? (Math.random() * 0.52) - 0.1 : 0; // Movimiento en X o quieto
