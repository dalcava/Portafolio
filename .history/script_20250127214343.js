...
function init() {
    ...
    let size = Math.min(canvas.width, canvas.height) * 0.015; // Tamaño proporcional al canvas

    for (let row = 0; row < numberOfRows; row++) {
        for (let col = 0; col < numberOfCols; col++) {
            let x = col * gridWidth + gridWidth / 12; // Posición X centrada en la celda
            let y = row * gridHeight + gridHeight / 12; // Posición Y centrada en la celda
