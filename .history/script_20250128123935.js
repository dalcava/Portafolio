...
function init() {
    particlesArray = [];
    let numberOfRows = 180; // Número de filas en la cuadrícula
    let numberOfCols = 45; // Número de columnas en la cuadrícula
    let gridWidth = canvas.width / numberOfCols; // Espaciado horizontal
    let gridHeight = canvas.height / numberOfRows; // Espaciado vertical
    let size = Math.min(canvas.width, canvas.height) * 0.0015; // Tamaño proporcional al canvas
...
const interactiveElements = document.querySelectorAll(".next-project-btn, .swiper-button-next, .swiper-button-prev, .swiper-pagination-bullet, .Lightbulb");
...
document.addEventListener("mousemove", (e) => {
    ...
    // Check if the target is a swiper slide
        ...
        customCursor.style.border = "1px solid rgba(188, 36, 74, 0.25)";
        customCursor.style.transition = "transform 0.15s ease-out, width 0.1s ease, background-color 0.3s ease, border-radius 0.3s ease";
