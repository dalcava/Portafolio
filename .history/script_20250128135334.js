...
const interactiveElements = document.querySelectorAll(".next-project-btn, .swiper-button-next, .swiper-button-prev, .swiper-pagination-bullet, .Lightbulb, .menu");
...
document.addEventListener("mousemove", (e) => {
    ...
    else if (target.classList.contains("Lightbulb") || target.classList.contains("menu")) {
        const rect = target.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;
