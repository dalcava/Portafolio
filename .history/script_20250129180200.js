...
document.querySelectorAll(".image-container, .imagen-contenida, .active-gif").forEach(element => {
    element.addEventListener("click", function () {
        const nextPage = "./Projects/Aval pay center/Aval pay center.html"; // Target page

        // Find the closest static image within the clicked container
        const image = this.closest(".swiper-slide").querySelector(".imagen-contenida");
        if (!image) return;
        ...
        gsap.to(clone, {
            ...
            borderRadius: "0px",
            onComplete: () => {
                gsap.to(clone, { opacity: 0, duration: 0.5, onComplete: () => window.location.href = nextPage });
