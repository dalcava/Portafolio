...
class Particle {
    ...
    update() {
        ...
        if (distance < mouse.radius) {
            ...
            this.color = interpolateColor('#BC244A', '#F6F6F6', t); 
        } else {
...
function init() {
    ...
    let size = Math.min(canvas.width, canvas.height) * 0.0015; 
...
/** 
 * NEW FUNCTION to check the window width and toggle the canvas.
...
document.addEventListener("DOMContentLoaded", function () {
    const personalPhotoContainer = document.querySelector('.personal-photo'); // Select the div
    const personalPhotoImg = document.querySelector('.personal-photo img'); // Select the img inside

    function navigateToUrl(event) {
        // Find the closest .personal-photo element (in case img is clicked)
        const parentDiv = event.target.closest('.personal-photo');
        
        if (parentDiv) {
            const url = parentDiv.getAttribute('data-url');
            console.log("Navigating to:", url || "No URL available");
    ...
    // Add click event to both the div and img
    if (personalPhotoContainer) {
        personalPhotoContainer.addEventListener('click', navigateToUrl);
    }
    if (personalPhotoImg) {
        personalPhotoImg.addEventListener('click', navigateToUrl);
    }
});
