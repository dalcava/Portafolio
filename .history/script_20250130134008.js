...
function crearPalabra() {
    ...
    gsap.to(palabraFrente, {
        clipPath: "inset(100% 0 0 0)", // Oculta el texto hacia arriba
        y: -50, // Mueve el texto hacia arriba
        duration: 0.25,
        onComplete: () => {
            // Cambia la palabra una vez que la animación fade-out termina
            palabraFrente.innerHTML = nuevaPalabra;

            // Animación de fade-in
            gsap.to(palabraFrente, {
                clipPath: "inset(0% 0 0 0)", // Muestra el texto desde abajo
                y: 0, // Lo devuelve a su posición original
                duration: 0.2
            });
        }
    });
...
/*             { opacity: 0, x: -300 }, // Start invisible and slightly below
            ...
            { clipPath: "inset(0% 0 0 0)", y:0, duration: 1, ease: "elastic.out(1, 0.85)" },
            "-=0"
        )
        .fromTo(
            ".control-container",
            { opacity: 0, y: 200, x: 0 }, // Start invisible and slightly below
            { opacity: 1, y: -45, x:0, duration: 1, ease: "elastic.out(1, 0.85)", stagger: 0.25 },
            "-=1" // Fade and move in
        )
        .fromTo(
            ".main-subtitle",
