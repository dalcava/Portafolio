...
document.querySelectorAll(".swiper-slide").forEach((slide) => {
    ...
    window.addEventListener("mousemove", (e) => {
        ...
        if (isCursorInsideSlide) {
            ...
            activeGif.style.maskImage = `radial-gradient(circle at ${x}% ${y}%, black 36.5%, transparent 37%)`;
            activeGif.style.webkitMaskImage = `radial-gradient(circle at ${x}% ${y}%, black 36.75%, transparent 37%)`;
    ...
    slide.addEventListener("mouseleave", () => {
        ...
        let interval = setInterval(() => {
            if (isInside || scale <= 0) {
                ...
                scale -= 1.5; // Gradually decrease the scale
                activeGif.style.maskImage = `radial-gradient(circle at 50% 50%, black ${scale}%, transparent ${scale + 1}%)`;
                activeGif.style.webkitMaskImage = `radial-gradient(circle at 50% 50%, black ${scale}%, transparent ${scale + 1}%)`;
            }
