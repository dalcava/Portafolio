...
document.querySelectorAll(".swiper-slide").forEach((slide) => {
    ...
    let lastX = 50; // Default X position (center)
    let lastY = 50; // Default Y position (center)
    let isInside = false;
    ...
    window.addEventListener("mousemove", (e) => {
        ...
        if (isCursorInsideSlide) {
            isInside = true;
            lastX = ((e.clientX - rect.left) / rect.width) * 100; // Store last X position
            ...
            activeGif.style.webkitMaskImage = `radial-gradient(circle at ${lastX}% ${lastY}%, black 36.75%, transparent 37%)`;
