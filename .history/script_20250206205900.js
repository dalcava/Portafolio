...
function updateSlidePositions() {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // If the slide is active or NOT prev and NOT next, center the image
        if (slide.classList.contains('swiper-slide-active') || 
            (!slide.classList.contains('swiper-slide-prev') && !slide.classList.contains('swiper-slide-next'))) {
            
            image.style.transform = 'translateX(0%)'; // Center the image smoothly
        } 
        else {
            // If it's prev or next, apply an offset with animation
...
function updateSlidePositions() {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // Apply a smoother cubic-bezier transition
        image.style.transition = 'transform 0.75s cubic-bezier(0.25, 1, 0.5, 1)';

        // If the slide is active or NOT prev and NOT next, center the image
        if (slide.classList.contains('swiper-slide-active') || 
            (!slide.classList.contains('swiper-slide-prev') && !slide.classList.contains('swiper-slide-next'))) {
            
            image.style.transform = 'translateX(0%)'; // Smooth centering
        } 
        else {
            // If it's prev or next, apply an offset smoothly
            const offset = (index - activeIndex) * -40; // Slightly reduced offset for better flow
            image.style.transform = `translateX(${offset}%)`;
        }
    });
}

// Apply smooth corrections at key moments
swiper.on('slideChangeTransitionStart', updateSlidePositions);
swiper.on('slideChangeTransitionEnd', updateSlidePositions);
swiper.on('transitionEnd', updateSlidePositions);

// Continuous smooth update while dragging
swiper.on('sliderMove', () => {
    requestAnimationFrame(updateSlidePositions);
});

// Final correction after user interaction stops
let correctionTimeout;
swiper.on('touchEnd', () => {
    clearTimeout(correctionTimeout);
    correctionTimeout = setTimeout(updateSlidePositions, 200); // Slight delay for extra smoothness
});

// 🔥 **MutationObserver for Real-time Fixing with Smooth Effect**
...
// Ensure proper reset on transition end (smoother easing)
swiper.on('slideChangeTransitionEnd', () => {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        if (index === activeIndex) {
            // Reset the active slide with a very smooth easing effect
            image.style.transition = 'transform 0.75s cubic-bezier(0.25, 1, 0.5, 1)';
            image.style.transform = 'translateX(0%)';
        } else {
            // Ensure inactive slides have a smooth transition
            const offset = (index - activeIndex) * -40;
            image.style.transition = 'transform 0.5s ease-out'; // Slightly softer easing
            image.style.transform = `translateX(${offset}%)`;
        }
    });
});
