...
function updateSlidePositions() {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // Determine offset (only applies to prev and next)
        const offset = (index - activeIndex) * -40; // Adjusted for smoother flow

        // Apply transformation
        image.style.transform = slide.classList.contains('swiper-slide-active') ? 
            'translateX(0%)' : `translateX(${offset}%)`;
    });
}
...
swiper.on('touchEnd', () => {
    ...
    correctionTimeout = setTimeout(updateSlidePositions, 200); // Extra smoothness
});

// 🔥 **MutationObserver for Real-time Fixing**
...
// Reset image position on slide grab (prevents sudden jumps)
swiper.on('sliderMove', () => {
