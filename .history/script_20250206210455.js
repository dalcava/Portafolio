...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        if (smooth) {
            ...
            image.style.transition = 'none'; // Disable transition if needed
        }

        // If the slide is active or NOT prev/next, center the image smoothly
...
// Ensure first swipe is smooth
swiper.on('slideChangeTransitionStart', () => {
    setTimeout(() => {
        updateSlidePositions(true); // Delay ensures no abrupt repositioning
    }, 10);
});

swiper.on('slideChangeTransitionEnd', () => updateSlidePositions(true));
swiper.on('transitionEnd', () => updateSlidePositions(true));

// Continuous smooth update while dragging
swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(false)); // Temporarily disable smoothness
});
...
swiper.on('touchEnd', () => {
    ...
    }, 200); // Small delay for extra smoothness
});
...
// Reset image position on slide grab (prevent abrupt snapping)
swiper.on('sliderMove', () => {
    document.querySelectorAll('.swiper-slide .imagen-contenida').forEach((image) => {
        image.style.transition = 'none'; // Temporarily remove transition
    });
});

// Ensure proper reset on transition end (avoid abrupt jumps)
