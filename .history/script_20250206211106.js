...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // If the slide is active or NOT prev and NOT next, center the image smoothly
        if (slide.classList.contains('swiper-slide-active') || 
            (!slide.classList.contains('swiper-slide-prev') && !slide.classList.contains('swiper-slide-next'))) {
            
            image.style.transition = smooth ? 'transform 0.55s cubic-bezier(0.25, 1, 0.5, 1)' : 'none';
            image.style.transform = 'translateX(0%)'; // Center the image
        } 
        else {
            // If it's prev or next, apply a smooth offset transition
            const offset = (index - activeIndex) * -45;
            image.style.transition = smooth ? 'transform 0.9s cubic-bezier(0.25, 1, 0.5, 1)' : 'none';
            image.style.transform = `translateX(${offset}%)`;
        }
    });
}

// Apply smooth corrections during transition
...
// Ensure final smooth correction after interaction stops
let correctionTimeout;
swiper.on('touchEnd', () => {
    clearTimeout(correctionTimeout);
    correctionTimeout = setTimeout(() => updateSlidePositions(true), 100); // Small delay for final correction
});

// 🔥 **Observe class changes for even smoother updates**
