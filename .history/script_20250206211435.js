...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // Hint the browser that 'transform' will change for smoother performance
        ...
        if (slide.classList.contains('swiper-slide-active') ||
            ...
            // Use a custom cubic-bezier for a smooth, natural feel
            image.style.transition = smooth ? 'transform 0.6s cubic-bezier(0.25, 1, 0.5, 1)' : 'none';
            image.style.transform = 'translateX(0%)';
        } else {
            // For slides that are prev or next, apply a smooth offset transition
            const offset = (index - activeIndex) * -45;
            image.style.transition = smooth ? 'transform 0.8s cubic-bezier(0.25, 1, 0.5, 1)' : 'none';
            image.style.transform = `translateX(${offset}%)`;
        }
    });
}

// Apply smooth corrections during transition events
...
// Final smooth correction after interaction stops
