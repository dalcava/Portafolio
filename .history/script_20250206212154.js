...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      ...
      // Enable GPU acceleration hints for silky-smooth transforms.
      image.style.willChange = 'transform';
      image.style.backfaceVisibility = 'hidden';
      image.style.transformStyle = 'preserve-3d';
  
      if (slide.classList.contains('swiper-slide-active')) {
        // For the active slide: center it, bring it forward (translateZ), and scale it up.
        image.style.transition = smooth
          ? 'transform 0.6s cubic-bezier(0.215, 0.61, 0.355, 1)'
          : 'none';
        image.style.transform = 'translate3d(0, 0, 30px) scale(1.05)';
      } else if (slide.classList.contains('swiper-slide-prev')) {
        // For the previous slide: offset it to the left, slightly tilt it, and scale it down.
        // Use the offset from the active slide for a fluid horizontal positioning.
        const offset = (index - activeIndex) * -45;
        image.style.transition = smooth
          ? 'transform 0.8s cubic-bezier(0.215, 0.61, 0.355, 1)'
          : 'none';
        image.style.transform = `translate3d(${offset}%, 0, 0) scale(0.95) rotateY(10deg)`;
      } else if (slide.classList.contains('swiper-slide-next')) {
        // For the next slide: offset it to the right, tilt it in the opposite direction, and scale it down.
        const offset = (index - activeIndex) * -45;
        image.style.transition = smooth
          ? 'transform 0.8s cubic-bezier(0.215, 0.61, 0.355, 1)'
          : 'none';
        ...
        // For any other slide (if any exist), revert to a neutral centered state.
        image.style.transition = smooth
          ? 'transform 0.6s cubic-bezier(0.215, 0.61, 0.355, 1)'
          : 'none';
  ...
  // Continuously update while the user is dragging.
  swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(true));
  });
  
  // Final correction after interaction stops.
  ...
  // Additionally, observe class changes on slides to ensure the effect remains fluid.
