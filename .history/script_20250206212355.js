...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
        ...
        // Optimize for GPU-accelerated, silky-smooth rendering.
        image.style.willChange = 'transform, filter';
        image.style.backfaceVisibility = 'hidden';
        image.style.transformStyle = 'preserve-3d';
        
        // If the slide is active or is not marked as prev/next, it should take center stage.
        if (
            slide.classList.contains('swiper-slide-active') ||
            (!slide.classList.contains('swiper-slide-prev') && !slide.classList.contains('swiper-slide-next'))
        ) {
            // Our active slide: centered, full-scale, with natural brightness.
            image.style.transition = smooth 
                ? 'transform 0.8s cubic-bezier(0.22, 1, 0.36, 1), filter 0.8s cubic-bezier(0.22, 1, 0.36, 1)'
                : 'none';
            image.style.transform = 'translate3d(0, 0, 0) scale(1)';
            image.style.filter = 'brightness(1) saturate(1)';
        } else {
            // For adjacent slides, apply an elegant offset with a gentle scale-down 
            // and a dreamy reduction in brightness and saturation.
            const offset = (index - activeIndex) * -50; // Adjust multiplier for the desired spacing effect.
            image.style.transition = smooth 
                ? 'transform 1s cubic-bezier(0.22, 1, 0.36, 1), filter 1s cubic-bezier(0.22, 1, 0.36, 1)'
                : 'none';
            image.style.transform = `translate3d(${offset}%, 0, 0) scale(0.95)`;
            image.style.filter = 'brightness(0.8) saturate(0.8)';
        }
    });
}

// Update positions at key Swiper events for fluidity
...
// Continuously update during dragging for real-time silky smoothness.
swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(true));
});

// Ensure a final graceful correction after touch interactions.
...
// Observe class changes on slides to capture any unexpected state changes and adjust gracefully.
