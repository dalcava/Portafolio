...
function updateSlidePositions(smooth = true) {
    ...
    // Loop over each slide in the Swiper
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      const image = slide.querySelector('.imagen-contenida');
      if (!image) return;
  
      // Hint to the browser for GPU acceleration and smoother rendering
      ...
      // Define GSAP animation parameters based on slide state:
      if (slide.classList.contains('swiper-slide-active')) {
        // The active slide: center it perfectly, full clarity, no blur, full scale.
        gsap.to(image, {
        ...
        // Non-adjacent slides: ensure they are centered and pristine.
        gsap.to(image, {
          duration: smooth ? 0.8 : 0,
          ease: 'power3.out',
          x: 0,
        ...
        // For adjacent slides (either prev or next):
        // • Offset them horizontally based on their position relative to the active slide.
        // • Slightly scale them down.
        // • Apply a subtle clip-path to create a framing effect.
        // • Add a tiny blur to gently de-emphasize them.
  ...
  // Apply smooth corrections during key Swiper events.
  ...
  // Continuously update while dragging so the transitions never break.
  swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(true));
  });
  
  // Final correction shortly after touch ends to ensure perfect alignment.
  ...
  // Use a MutationObserver to catch any class changes (from Swiper’s internal updates) and re-apply our transitions.
