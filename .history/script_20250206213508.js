...
// Ensure Swiper is initialized with watchSlidesProgress enabled, e.g.:
// const swiper = new Swiper('.swiper', { watchSlidesProgress: true, ... });

function updateSlidePositions(smooth = true) {
    const activeIndex = swiper.activeIndex;
    // Define a parallax factor (0.5 means the image moves at half the speed of the slide)
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      ...
      // animate it to be perfectly centered with no parallax.
      if (
        ...
        // For adjacent slides (prev or next), calculate the base offset
        const offset = (index - activeIndex) * -45;
        // Apply a parallax effect: the image moves by a fraction of the slide's offset.
        const parallaxOffset = offset * parallaxFactor;
    
        gsap.to(image, {
          duration: smooth ? 1 : 0,
          ease: 'power3.out',
          x: `${parallaxOffset}%`,   // Apply parallax offset
  ...
  // Continuously update while dragging so that the transitions remain fluid.
  // Here, we use immediate updates for responsiveness (no tween duration).
  ...
  // Use a MutationObserver to catch any class changes and update transitions accordingly.
