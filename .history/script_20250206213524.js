...
// Make sure you initialize Swiper with watchSlidesProgress enabled:
// const swiper = new Swiper('.swiper', { watchSlidesProgress: true, ... });

function updateSlidePositions(smooth = true) {
    // Loop over each slide and update its image based on the slide's progress
    document.querySelectorAll('.swiper-slide').forEach(slide => {
      const image = slide.querySelector('.imagen-contenida');
      if (!image) return;
  
      // Hint the browser that these properties will change for smoother rendering
      image.style.willChange = 'transform, clip-path, filter';
  
      // Use the slide's progress (a continuous value set by Swiper)
      // If watchSlidesProgress is enabled, each slide's "progress" property
      // will be updated in real time as the user drags.
      let progress = slide.progress || 0; // progress typically ranges between -1 and 1
      // Calculate the horizontal offset using the progress.
      // A factor of -45 makes the effect noticeable but can be adjusted.
      let offset = progress * -45;
  
      // For the active (centered) slide (progress near 0), remove any offset,
      // clipping, or blur effects.
      if (Math.abs(progress) < 0.01) {
        ...
        // For slides that are off-center, animate them with an offset,
        // a soft clip-path (to "frame" the image), and a subtle blur.
        gsap.to(image, {
          duration: smooth ? 1 : 0,
          ease: 'power3.out',
          x: `${offset}%`,
  ...
  // Update during key Swiper events for continuous smooth motion:
  ...
  // While dragging, update continuously.
  // During dragging you might want immediate (non-smooth) updates for responsiveness.
  swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(false));
  });
  
  // After the touch (or drag) ends, allow the smooth transition to finalize.
  ...
  // Optionally, use a MutationObserver to catch any class changes (if needed)
  // to trigger an update—this further guarantees that the visual state is correct.
