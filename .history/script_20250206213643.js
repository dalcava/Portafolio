...
// Make sure Swiper is initialized with watchSlidesProgress enabled, for example:
...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      ...
      // For active or non-adjacent slides, keep the image centered and clear
      if (
        ...
        // • Calculate a base offset relative to the active slide.
        // • Add a subtle parallax offset based on the slide's progress (a value between -1 and 1).
        const baseOffset = (index - activeIndex) * -45;
        const parallaxOffset = (slide.progress || 0) * 10; // Adjust "10" to fine-tune the parallax strength
        const finalOffset = baseOffset + parallaxOffset;
        gsap.to(image, {
          duration: smooth ? 1 : 0,
          ease: 'power3.out',
          x: `${finalOffset}%`, // Combine base offset with parallax effect
          clipPath: 'inset(5% 5% 5% 5%)', // Soft clipping for an elegant framing look
          filter: 'blur(1px)' // A gentle blur to de-emphasize off-center slides
        });
      }
    });
  }
  ...
  // Continuously update while dragging for a fluid, real‑time effect.
  // During dragging we set smooth to false for immediate responsiveness.
  swiper.on('sliderMove', () => {
    requestAnimationFrame(() => updateSlidePositions(false));
  });
  
  // After touch/drag ends, allow a smooth correction to finalize the positions.
  ...
  // Optionally, use a MutationObserver to catch any class changes on slides
  // and update transitions accordingly for extra robustness.
