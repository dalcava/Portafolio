...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      ...
      // For active or non-adjacent slides, center the image and remove effects.
      if (
        ...
        // - Calculate a base offset.
        // - Add a very subtle parallax offset based on the slide's progress.
        const baseOffset = (index - activeIndex) * -45;
        const parallaxOffset = (slide.progress || 0) * 3; // Just a little parallax effect
        const finalOffset = baseOffset + parallaxOffset;
  
        gsap.to(image, {
          duration: smooth ? 1 : 0,
          ease: 'power3.out',
          x: `${finalOffset}%`,    // Combine base offset with a subtle parallax shift
  ...
  // Optionally, use a MutationObserver to detect class changes and update transitions accordingly
