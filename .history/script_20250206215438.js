...
ScrollTrigger.create({
  trigger: ".swiper-container",      // The container to pin
  start: "top top",                  // When the container reaches the top of the viewport
  // Set the end so that each slide gets a chunk of scroll distance (adjust the multiplier as needed)
  end: `+=${totalSlides * 1000}`,     
  pin: true,                         // Pin the container during the scroll
  scrub: 1.5,                        // Scrub (link scroll position to animation progress); higher values = slower response
  ease: "power2.out",                // Easing for a gentle transition

  onUpdate: self => {
    // Compute the new slide index based on scroll progress (value between 0 and 1)
    let slideIndex = Math.round(self.progress * (totalSlides - 1));
    // If the computed slide is different than the active one, change it.
