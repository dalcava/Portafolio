...
const throttleDelay = 0; // Minimum delay between wheel events (in ms)
...
const navigateSwiper = (direction) => {
  ...
  isAnimating = true;
  // Use GSAP.delayedCall to wait for the slide transition to finish (e.g., 1.2 seconds)
...
window.addEventListener("wheel", (e) => {
  const now = Date.now();
  if (now - scrollThrottle < throttleDelay) return;
  scrollThrottle = now;
  const direction = Math.sign(e.deltaY);
  navigateSwiper(direction);
});
ScrollTrigger.create({
    trigger: ".scroll-container",  // The scrollable element (ensure it’s tall enough)
    start: "top top",              // When the container’s top meets the viewport’s top
    end: "bottom top",             // When the container’s bottom meets the viewport’s top
    scrub: 0.5,                    // Smooth scrubbing over time (adjust for desired smoothness)
    onUpdate: (self) => {
      // Map the scroll progress (0 to 1) to half the carousel’s progress (0 to 0.5)
      const newProgress = self.progress * 0.5;
      // Immediately update the swiper’s progress with no extra tweening (second parameter = 0)
      swiper.setProgress(newProgress, 0);
    }
  });
