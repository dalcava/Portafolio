...
gsap.registerPlugin(ScrollTrigger);
    ...
    // Create a ScrollTrigger instance that maps scroll progress to a subtle horizontal translation
    ScrollTrigger.create({
      trigger: ".scroll-container",  // The scrollable container
      start: "top top",              // When the container’s top reaches the viewport top
      end: "bottom top",             // When the container’s bottom reaches the viewport top
      scrub: 0.5,                    // Smoothly scrubs the animation in sync with scroll (adjust as needed)
      onUpdate: (self) => {
        // Map the scroll progress (0 to 1) to a subtle horizontal translation (0% to -25%)
        // Adjust the multiplier (25) to control the effect strength.
        const newProgress = self.progress * 25;
        // Update the target's x transform using GSAP
        gsap.to(".your-target", { 
          x: `-${newProgress}%`, 
          ease: "none", 
          duration: 0 
