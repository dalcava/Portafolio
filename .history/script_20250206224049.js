...
window.addEventListener('load', () => {
    // Creamos una línea de tiempo para animar cada punto individualmente.
    const tl = gsap.timeline({
      onComplete: () => {
        // Cuando todas las animaciones hayan finalizado, ocultamos la pantalla de carga.
    ...
    // Definimos un array con las duraciones deseadas para cada punto (en segundos).
    const durations = [0.8, 1.0, 0.6, 1.2]; // Puedes ajustar estos valores según prefieras
  
    // Animamos cada punto para que se desplace hacia abajo.
    // Todos comienzan a la vez (posición 0 en la línea de tiempo), pero con duraciones diferentes.
    dots.forEach((dot, index) => {
      tl.to(dot, { 
        y: "150%", // Mueve el punto hacia abajo (fuera de la pantalla)
