...
document.querySelectorAll(".swiper-slide").forEach((slide) => {
  ...
  window.addEventListener("mousemove", (e) => {
      ...
      if (isCursorInsideSlide) {
          ...
          activeGif.style.maskImage = `radial-gradient(circle at ${lastX}% ${lastY}%, black 25%, transparent 45%)`;
          activeGif.style.webkitMaskImage = `radial-gradient(circle at ${lastX}% ${lastY}%, black 25%, transparent 45%)`;

          activeGif.style.opacity = "1";
          staticImg.style.opacity = "1";
          staticImg.style.filter = "brightness(0.75)"; // Dim the static image
      }
  });
