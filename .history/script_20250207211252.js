...
// Function to toggle between eye and scroll SVG
function morphCursor() {
    gsap.to(cursor, {
        duration: 1, // Transition duration
        morphSVG: cursor.dataset.state === "eye"
            ? "./Slider/Recursos/scroll.svg"
            : "./Slider/Recursos/Eye.svg", // Toggle between SVGs
        ease: "power2.inOut",
        onComplete: () => {
            // Toggle state
            cursor.dataset.state = cursor.dataset.state === "eye" ? "scroll" : "eye";
        }
    });
}

// Run the animation every 2 seconds
setInterval(morphCursor, 2000);
