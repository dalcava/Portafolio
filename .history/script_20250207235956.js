...
window.addEventListener('resize', function () {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    mouse.radius = ((canvas.height/35) * (canvas.height/35));
...
function updateSlidePositions(smooth = true) {
    ...
    document.querySelectorAll('.swiper-slide').forEach((slide, index) => {
      ...
      if (
        ...
        gsap.to(image, {
          ...
          clipPath: 'inset(0% 5% 0% 5%)' // Soft clipping for a refined look
        });
      }
    });
  }
  swiper.on('slideChange', function () {
    gsap.fromTo(".swiper-pagination-bullet-active", 
  ...
  swiper.on('slideChange', function () {
    ...
    document.querySelectorAll('.swiper-pagination-bullet').forEach(bullet => {
        bullet.innerHTML = ""; // Clear previous shapes
    });

    // Add a shape in front of the active bullet
    let activeBullet = document.querySelector('.swiper-pagination-bullet-active');
    if (activeBullet) {
        let shape = document.createElement('div');
        shape.classList.add('pagination-shape');
        activeBullet.prepend(shape); // Inserts shape before bullet

        // GSAP animation for growing effect (width 100% in 8s)
        gsap.fromTo(shape, 
            { width: "12px" }, 
            { width: "72px", duration: 5.3, ease: "ease in" }
        );
    }

    // Restart autoplay when the slide change is complete
...
document.querySelectorAll(".swiper-slide").forEach((slide) => {
  ...
  window.addEventListener("mousemove", (e) => {
      ...
      if (isCursorInsideSlide) {
          ...
          staticImg.style.filter = "brightness(0.85) saturate(0.5)"; // Dim the static image
      }
  });
...
const interactiveElements = document.querySelectorAll(
  ".blur, .next-project-btn, .swiper-button-next, .swiper-button-prev, .swiper-slide-next, .swiper-pagination-bullet, .Lightbulb, .menu, .icon-buttons"
);
...
document.addEventListener("mousemove", (e) => {
    ...
    // Check if the target is a swiper slide
    else if (target.classList.contains("active-gif") || target.classList.contains("imagen-contenida") && !target.classList.contains("swiper-slide-next")) {    
      ...
      customCursor.style.borderRadius = "32px";
      customCursor.style.scale = ".75";
    }
    else if (target.classList.contains("blur")) {    
      customCursor.style.width = "64px";
      customCursor.style.height = "32px";
      customCursor.style.backgroundImage = "url('./Slider/Recursos/Scroll.svg')";
      customCursor.style.backgroundRepeat = "no-repeat";
      customCursor.style.backgroundPosition = "center";
      customCursor.style.opacity = "1";
      customCursor.style.transform = "translate(-50%, -50%) scale(1.5)";
